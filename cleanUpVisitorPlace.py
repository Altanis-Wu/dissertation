...
for i in range(2011, 2020):
    input = []
    output = []
    f = open('original_data/original_visitor_place/original_visitor_place_' + str(i) + '.csv', 'r')
    ...
    f = open('transfer/visitor_place/visitor_place_' + str(i) + '.csv', 'w')
    for line in output:
        f.write(line)
    f.close()
for i in range(2011, 2016):
    #df = pd.read_csv('transfer/visitor_activity/visitor_activity_'+str(i)+'.csv')
    df = pd.read_csv('transfer/visitor_place/visitor_place_' + str(i) + '.csv')
    df.columns = ['place', 'total', 'male', 'female', '16-24', '25-34', '35-44', '45-54', '55-64', '65+', 'married', 'not married', 'yes', 'no', 'access to car (1+)', 'no access to car (0)', 'ab', 'c1', 'c2', 'de', 'employed/self-employed (full or part time)', 'in full or part time education', 'unemployed/not working', '']
    df.to_csv('transfer/visitor_place/visitor_place_' + str(i) + '.csv', index = 0)
#data = pd.read_csv('transfer/visitor_activity/visitor_activity_2011.csv')
data = pd.read_csv('transfer/visitor_place/visitor_place_2011.csv')
data['year'] = 2011
...
age.to_csv('data/age/age_place.csv', index=0)
gender.to_csv('data/gender/gender_place.csv', index=0)
married.to_csv('data/married/married_place.csv', index=0)
children.to_csv('data/children/children_place.csv', index=0)
cars.to_csv('data/cars/cars_place.csv', index=0)
social.to_csv('data/social/social_place.csv', index=0)
working.to_csv('data/working/working_place.csv', index=0)