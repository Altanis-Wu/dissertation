...
for i in range(2011, 2020):
    input = []
    output = []
    f = open(os.path.dirname(os.getcwd())+'/original_data/original_visitor/original_visitor_'+str(i)+'.csv', 'r')
    ...
    for line in input:
        ...
        if line[0] == '\"\"' and line[1] == '\"total\"':
            ...
            output.append(','.join(x for x in line))
    f = open(os.path.dirname(os.getcwd())+'/transfer/visitor/visitor_'+str(i)+'.csv', 'w')
    for line in output:
        f.write(line)
    f.close()
for i in range(2011, 2016):
    df = pd.read_csv(os.path.dirname(os.getcwd())+'/transfer/visitor/visitor_'+str(i)+'.csv')
    ...
    df.to_csv(os.path.dirname(os.getcwd())+'/transfer/visitor/visitor_'+str(i)+'.csv', index = 0)
data = pd.read_csv(os.path.dirname(os.getcwd())+'/transfer/visitor/visitor_2011.csv')
data['year'] = 2011
...
age.to_csv(os.path.dirname(os.getcwd())+'/data/age/age.csv', index=0)
gender.to_csv(os.path.dirname(os.getcwd())+'/data/gender/gender.csv', index=0)
married.to_csv(os.path.dirname(os.getcwd())+'/data/married/married.csv', index=0)
children.to_csv(os.path.dirname(os.getcwd())+'/data/children/children.csv', index=0)
cars.to_csv(os.path.dirname(os.getcwd())+'/data/cars/cars.csv', index=0)
social.to_csv(os.path.dirname(os.getcwd())+'/data/social/social.csv', index=0)
working.to_csv(os.path.dirname(os.getcwd())+'/data/working/working.csv', index=0)